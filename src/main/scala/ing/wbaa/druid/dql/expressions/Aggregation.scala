...
final case class HLLSketchBuildAgg(
    fieldName: String,
    name: Option[String] = None,
    lgK: Int = 12,
  ...
  override protected[dql] def build(): Aggregation =
    HLLSketchBuildAggregation(this.getName, fieldName, lgK, tgtHllType)

  override def alias(name: String): AggregationExpression = copy(name = Option(name))

  override def getName: String = name.getOrElse(s"hll_build_$fieldName")

  def lgK(v: Int): HLLSketchBuildAgg            = copy(lgK = v)
  def tgtHllType(v: HLLType): HLLSketchBuildAgg = copy(tgtHllType = v)

  def set(lgK: Int = 12, tgtHllType: HLLType = HLLType.HLL_4): HLLSketchBuildAgg =
    copy(lgK = lgK, tgtHllType = tgtHllType)
}

final case class HLLSketchMergeAgg(
    fieldName: String,
    name: Option[String] = None,
    lgK: Int = 12,
    tgtHllType: HLLType = HLLType.HLL_4
  ...
  override protected[dql] def build(): Aggregation =
    HLLSketchMergeAggregation(this.getName, fieldName, lgK, tgtHllType)

  override def alias(name: String): AggregationExpression = copy(name = Option(name))

  override def getName: String = name.getOrElse(s"hll_merge_$fieldName")

  def lgK(v: Int): HLLSketchMergeAgg            = copy(lgK = v)
  def tgtHllType(v: HLLType): HLLSketchMergeAgg = copy(tgtHllType = v)

  def set(lgK: Int = 12, tgtHllType: HLLType = HLLType.HLL_4): HLLSketchMergeAgg =
    copy(lgK = lgK, tgtHllType = tgtHllType)
}
